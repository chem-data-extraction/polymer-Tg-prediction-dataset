...
Reads the manifest at `specs/pdf_extraction_manifest.json` and, for each paper:

  1. Downloads the PDF into `data/raw/` if it is not yet present.
  2. Locates Table 3 with pdfplumber (and PyMuPDF as a fallback for text search).
  3. Extracts the Tg column with provenance (page, table caption, evidence text).
  4. Writes per-source CSV files into `data/extracted/` matching the dataset
     schema PLUS provenance columns (`page`, `table_id`, `evidence_text`,
     `review_status`).
  5. Appends one JSONL entry per record into `data/extracted/extraction_log.jsonl`.

The script is intentionally conservative: anything that cannot be unambiguously
read from a Table 3 cell is written with `tg_value=` empty and
`review_status="needs_pdf_verification"`, so that Practice 5 cleaning can decide
whether to keep, drop, or manually fill those rows.
...
If PyMuPDF / pdfplumber are not installed, the script falls back to a curated
table (`CURATED_TG_TABLE` below) that was built from the article HTML and
verified against the surrounding paper text. This keeps Practice 3 reproducible
and lets `data/extracted/` be regenerated without network access.
"""
...
# ---------------------------------------------------------------------------
# Curated Table 3 contents (the ground truth used in Practice 3)
#
# Values marked as "verified_from_text" are taken from the article's own
# discussion text and therefore are reliable irrespective of Table 3 cell
# parsing. Values marked "needs_pdf_verification" are present in Table 3 of the
# PDF but not exposed in the HTML rendering that was available during this
# Practice — they should be cross-checked by opening the actual PDF and
# rerunning this script with `--use-pdf` once the PDFs are downloaded.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Live PDF extraction (used when --use-pdf flag is passed and PyMuPDF /
# pdfplumber are installed). Falls back gracefully otherwise.
# ---------------------------------------------------------------------------
...
def download_pdf(url: str, dest: Path) -> bool:
    ...
    try:
        ...
        response = requests.get(url, timeout=60, headers={"User-Agent": "PolymerTg/0.1"})
...
def main() -> int:
    ...
    for target in manifest["targets"]:
        ...
        if local_pdf.exists():
            ...
            pdf_status[target["source_id"]] = "downloaded" if ok else "missing_offline_mode"
    ...
    print("==============================")
    print(f"  Combined CSV: {combined_path.relative_to(ROOT)}  ({len(records)} records)")
